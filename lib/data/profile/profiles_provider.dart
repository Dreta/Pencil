...
class ProfilesProvider extends ChangeNotifier {
  ...
  Future<void> downloadProfile(BuildContext context, Profile profile) async {
    ...
    try {
      ...
      Version version = await downloadVersion(context, client, profile.version, host, manifest.manifest!, tasks, task);
      Assets assets = await downloadAssetIndex(context, client, version, host, tasks, task);
      await downloadLoggingConfig(context, client, version, host, tasks, task);
      await downloadAssets(context, client, assets, version.assetIndex.totalSize, host, tasks, task);
      await downloadLibraries(context, client, version, profile, host, tasks, task);
      await downloadGame(context, client, version, host, task, tasks);
    } catch (e) {
      showDialog(
          context: context,
  ...
  Future<Version> downloadVersion(BuildContext context, http.Client client, String version, Host host, VersionManifest manifest,
  ...
  Future<Assets> downloadAssetIndex(
  ...
  Future<void> downloadAssets(
    ...
    task.progress = 0;
    tasks.notify();
    await downloadAsset(context, client, host, assets.objects.entries.toList(), 0, totalSize, 0, tasks, task);
  }

  Future<void> downloadAsset(BuildContext context, http.Client client, Host host, List<MapEntry<String, Asset>> assets,
    ...
    if (fileBytes.lengthInBytes == (asset.value.size ?? fileBytes.lengthInBytes) &&
      ...
      tasks.notify();
      return await downloadAsset(context, client, host, assets, totalDownloaded, totalSize, current + 1, tasks, task);
    }
    ...
    r.stream.listen((value) {
      ...
      ..onDone(() async {
        ...
        await downloadAsset(context, client, host, assets, totalDownloaded, totalSize, current + 1, tasks, task);
        completer.complete();
      });
    return completer.future;
  }

  Future<void> downloadLoggingConfig(
      BuildContext context, http.Client client, Version version, Host host, TasksProvider tasks, Task task) async {
    SettingsProvider settings = Provider.of<SettingsProvider>(context, listen: false);
  ...
  Future<void> downloadLibraries(BuildContext context, http.Client client, Version version, Profile profile, Host host,
      TasksProvider tasks, Task task) async {
    ...
    await downloadLibrary(context, client, host, toDownload, nativeNames, 0, 0, totalSize, task, tasks);
  }

  Future<void> downloadLibrary(BuildContext context, http.Client client, Host host, List<Library> libraries,
